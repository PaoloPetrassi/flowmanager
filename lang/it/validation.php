<?php

return [
    'accepted' => 'Il campo :attribute deve essere accettato.',
    'after_or_equal' => 'Il campo :attribute deve essere una data successiva o uguale a :date.',
    'array' => 'Il campo :attribute deve essere un array.',
    'before_or_equal' => 'Il campo :attribute deve essere una data precedente o uguale a :date.',
    'boolean' => 'Il campo :attribute deve essere vero o falso.',
    'confirmed' => 'La conferma del campo :attribute non corrisponde.',
    'date' => 'Il campo :attribute deve contenere una data valida.',
    'email' => 'Il campo :attribute deve contenere un indirizzo email valido.',
    'enum' => 'Il valore selezionato per :attribute non è valido.',
    'exists' => 'Il valore selezionato per :attribute non è valido.',
    'integer' => 'Il campo :attribute deve essere un numero intero.',
    'max' => [
        'array' => 'Il campo :attribute non può contenere più di :max elementi.',
        'file' => 'Il file :attribute non può superare :max kilobyte.',
        'numeric' => 'Il campo :attribute non può essere superiore a :max.',
        'string' => 'Il campo :attribute non può superare :max caratteri.',
    ],
    'mimes' => 'Il campo :attribute deve essere un file di tipo: :values.',
    'min' => [
        'array' => 'Il campo :attribute deve contenere almeno :min elementi.',
        'file' => 'Il file :attribute deve essere di almeno :min kilobyte.',
        'numeric' => 'Il campo :attribute deve essere almeno :min.',
        'string' => 'Il campo :attribute deve contenere almeno :min caratteri.',
    ],
    'numeric' => 'Il campo :attribute deve essere un numero.',
    'required' => 'Il campo :attribute è obbligatorio.',
    'size' => [
        'array' => 'Il campo :attribute deve contenere :size elementi.',
        'file' => 'Il file :attribute deve essere di :size kilobyte.',
        'numeric' => 'Il campo :attribute deve essere :size.',
        'string' => 'Il campo :attribute deve contenere :size caratteri.',
    ],
    'string' => 'Il campo :attribute deve essere una stringa.',
    'unique' => 'Il valore del campo :attribute è già stato utilizzato.',
    'url' => 'Il campo :attribute deve contenere un URL valido.',

    'attributes' => [
        'address' => 'indirizzo',
        'asset_tag' => 'codice bene',
        'assigned_to' => 'assegnatario',
        'brand' => 'marca',
        'body' => 'commento',
        'budget' => 'budget',
        'category' => 'categoria',
        'city' => 'città',
        'code' => 'codice',
        'company_id' => 'azienda',
        'contact_id' => 'contatto',
        'country_code' => 'paese',
        'department' => 'reparto',
        'description' => 'descrizione',
        'due_date' => 'data di scadenza',
        'email' => 'email',
        'employees' => 'dipendenti',
        'file' => 'file',
        'first_name' => 'nome',
        'industry' => 'settore',
        'is_primary' => 'contatto principale',
        'job_title' => 'ruolo lavorativo',
        'last_name' => 'cognome',
        'legal_name' => 'ragione sociale',
        'manager_id' => 'responsabile progetto',
        'milestone_id' => 'milestone',
        'parent_id' => 'attività padre',
        'dependency_ids' => 'dipendenze',
        'estimated_minutes' => 'stima in minuti',
        'progress_override' => 'avanzamento manuale',
        'recurrence' => 'ricorrenza',
        'recurrence_interval' => 'intervallo di ricorrenza',
        'recurrence_ends_at' => 'fine ricorrenza',
        'minutes' => 'minuti',
        'date' => 'data',
        'role' => 'ruolo nel progetto',
        'trigger' => 'evento automazione',
        'action' => 'azione automazione',
        'action_user_id' => 'utente destinatario',
        'action_priority' => 'priorità da impostare',
        'condition_priority' => 'condizione di priorità',
        'mobile' => 'cellulare',
        'model' => 'modello',
        'name' => 'nome',
        'notes' => 'note',
        'password' => 'password',
        'phone' => 'telefono',
        'postal_code' => 'CAP',
        'priority' => 'priorità',
        'project_id' => 'progetto',
        'province' => 'provincia',
        'purchase_cost' => 'costo di acquisto',
        'purchase_date' => 'data di acquisto',
        'reference' => 'riferimento',
        'resolution' => 'risoluzione',
        'roles' => 'ruoli',
        'serial_number' => 'numero seriale',
        'slug' => 'slug',
        'start_date' => 'data di inizio',
        'status' => 'stato',
        'subject' => 'oggetto',
        'tax_code' => 'codice fiscale',
        'title' => 'titolo',
        'type' => 'tipo',
        'vat_number' => 'partita IVA',
        'warranty_expires_at' => 'scadenza garanzia',
        'website' => 'sito web',
    ],
];
