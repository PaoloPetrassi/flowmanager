...
<div class="row g-3">
    ...
    <div class="col-12 col-md-6">
        ...
        <select id="company_id" name="company_id" class="form-select @error('company_id') is-invalid @enderror" required><option value="">{{ __('Select a company') }}</option>@foreach ($companies as $company)<option value="{{ $company->id }}" @selected((string) old('company_id', $project->company_id) === (string) $company->id)>{{ $company->name }}</option>@endforeach</select>
        @error('company_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="col-12 col-md-6">
        <label for="contact_id" class="form-label fw-semibold">{{ __('Reference contact') }}</label>
        <select id="contact_id" name="contact_id" class="form-select @error('contact_id') is-invalid @enderror" data-company-contact-select data-company-source="company_id"><option value="">{{ __('No reference contact') }}</option>@foreach ($contacts as $contact)<option value="{{ $contact->id }}" data-company-id="{{ $contact->company_id }}" @selected((string) old('contact_id', $project->contact_id) === (string) $contact->id)>{{ $contact->full_name }}{{ $contact->company ? ' — '.$contact->company->name : '' }}</option>@endforeach</select>
    ...
    <div class="col-12 col-md-6">
        ...
        <select id="manager_id" name="manager_id" class="form-select @error('manager_id') is-invalid @enderror"><option value="">{{ __('Unassigned') }}</option>@foreach ($managers as $manager)<option value="{{ $manager->id }}" @selected((string) old('manager_id', $project->manager_id) === (string) $manager->id)>{{ $manager->name }} — {{ $manager->email }}</option>@endforeach</select>
    ...
    <div class="col-12 col-md-3">
        ...
        <div class="input-group"><span class="input-group-text">€</span><input id="budget" name="budget" type="number" min="0" step="0.01" class="form-control @error('budget') is-invalid @enderror" value="{{ old('budget', $project->budget) }}">@error('budget')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
    </div>
    <div class="col-12 col-md-3">
        <label for="estimated_minutes" class="form-label fw-semibold">{{ __('Estimated effort') }}</label>
        <div class="input-group"><input id="estimated_minutes" name="estimated_minutes" type="number" min="0" step="30" class="form-control @error('estimated_minutes') is-invalid @enderror" value="{{ old('estimated_minutes', $project->estimated_minutes) }}"><span class="input-group-text">{{ __('min') }}</span>@error('estimated_minutes')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
    </div>
    <div class="col-12 col-md-3">
        <label for="progress_override" class="form-label fw-semibold">{{ __('Manual progress') }}</label>
        <div class="input-group"><input id="progress_override" name="progress_override" type="number" min="0" max="100" class="form-control @error('progress_override') is-invalid @enderror" value="{{ old('progress_override', $project->progress_override) }}" placeholder="{{ __('Automatic') }}"><span class="input-group-text">%</span>@error('progress_override')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
        <div class="form-text">{{ __('Leave empty to calculate progress from completed tasks.') }}</div>
    </div>
    <div class="col-12">
